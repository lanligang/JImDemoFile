 ...
 * 发送消息 声音 、文字 、图片 消息
 */
- (void)chatBox:(ZXChatBoxView *)chatBox sendMessage:(id)message
{
...
- (void) imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    //压缩图片
    UIImage * compressionImage =  [self compressImageQuality:image toByte:1000*500];
    
    if (_delegate && [_delegate respondsToSelector:@selector(chatBoxViewController: sendMessage:)]) {
        //暂时调用该参数
        [_delegate chatBoxViewController:self sendMessage:compressionImage];
    }
...
- (ZXChatBoxView *) chatBox
{
    // 6 的初始化 0.0.375.49
    if (_chatBox == nil) {
        _chatBox = [[ZXChatBoxView alloc] initWithFrame:CGRectMake(0, 0, WIDTH_SCREEN, 49.0)];
