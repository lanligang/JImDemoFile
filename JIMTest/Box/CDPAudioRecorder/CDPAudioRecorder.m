...
@interface CDPAudioRecorder () <AVAudioRecorderDelegate,AVAudioPlayerDelegate> {
...
//    [self deleteAudioFile];
 ...
 //记录开始的时间戳
   self.startTimeLine = [[NSDate date]timeIntervalSince1970];
...
+(BOOL)convertAMRtoWAV:(NSString *)fielPath savePath:(NSString *)savePath{
    NSData *data=[NSData dataWithContentsOfURL:[NSURL URLWithString:fielPath]];
...
-(void)audioRecorderDidFinishRecording:(AVAudioRecorder *)recorder successfully:(BOOL)flag{
 ...
 if ([_delegate respondsToSelector:@selector(recordFinishWithRecordeModel:isSuccess:)])
 {
           RecorderModel *model = [RecorderModel new];
  if (flag){
      //计算出来的是秒的百分位
      model.duration = ([[NSDate date] timeIntervalSince1970] - self.startTimeLine)*100/1;
      model.localUrl = [self wavVoicePath];
	  model.voiceSize = [CDPAudioRecorder fileSizeAtPath:_recordURL.absoluteString];
  }
     //转码
     [self convertAudioToMP3WithPath:[NSURL fileURLWithPath:[self wavVoicePath]]];
     
     [_delegate recordFinishWithRecordeModel:model isSuccess:flag];
     
 }
}

//这里只是记录发送的文件 不去管理文件
-(NSString *)wavVoicePath
{
...
-(void)createTimer{
    if (_timer==nil)
    {
        _timer = [NSTimer timerWithTimeInterval:0.01 target:self selector:@selector(volumeMeters:) userInfo:nil repeats:YES];
 ...
 * 转码成MP3文件
 */
- (void)convertAudioToMP3WithPath:(NSURL *)saveURL
{
    //saveURL  MP3的存储地址
    @try {
        int read, write;
        FILE *pcm = fopen([[self getAudioRecordPath] cStringUsingEncoding:1], "rb");
        if (pcm == NULL) return;
        fseek(pcm, 4*1024, SEEK_CUR);   // Skip file header
        
        FILE *mp3 = fopen([saveURL.path cStringUsingEncoding:1], "wb");
        ...
        NSLog(@"转码出现问题 CONVERT MP3 EXCEPTION: %@",[exception description]);
    } @finally {
